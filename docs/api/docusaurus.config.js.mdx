...
export default {
  tagline:
    'Docusaurus를 사용하면 오픈 소스 문서 웹사이트를 쉽게 유지 관리할 수 있습니다.',
};
```
...
export default {
  themeConfig: {
    ...
    footer: {
      ...
      copyright: `Copyright © ${new Date().getFullYear()} Facebook, Inc.`, // 여기에 자체 HTML을 넣을 수도 있습니다
    },
  },
};
```
...
type MarkdownConfig = {
  ...
  remarkRehypeOptions: object; // https://github.com/remarkjs/remark-rehype#options 참조
...
애플리케이션을 렌더링하는 데 사용될 [Eta의 구문](https://eta.js.org/docs/syntax#syntax-overview)으로 작성된 HTML 템플릿입니다. 이를 사용하여 `body` 태그에 사용자 정의 속성을 설정하거나, 추가 `meta` 태그를 설정하거나, `viewport`를 사용자 정의하는 등의 작업을 할 수 있습니다. Docusaurus가 제대로 작동하려면 템플릿이 올바르게 구조화되어 있어야 한다는 점에 유의하세요. 일단 사용자 정의하면 템플릿이 상위 요구 사항을 준수하는지 확인해야 합니다.
